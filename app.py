...
def get_reports():
    ...
    if log_id:
        ...
        query = "SELECT log_id, scrape_date, url, change_type, summary, summary as plain_summary, source_tag, importance FROM change_log WHERE log_id = ?"
        params = [log_id]
    else:
        ...
        if search_query:
            ...
            query = f"""
                SELECT log_id, scrape_date, url, change_type, summary, summary as plain_summary, source_tag, importance 
                FROM change_log 
        ...
        if importance_filter:
            importance_values = [value.strip() for value in importance_filter.split(',') if value.strip()]
            if importance_values:
                placeholders = ',' .join('?' for _ in importance_values)
                query += f" AND importance IN ({placeholders})"
                params.extend(importance_values)
