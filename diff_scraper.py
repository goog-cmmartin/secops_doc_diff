...
def setup_database():
    ...
    if 'importance' not in columns:
        cursor.execute('ALTER TABLE change_log ADD COLUMN importance TEXT')
...
def get_all_links(url, base_url, session, current_page_url=None):
    links = set()
    broken_links = set()
    try:
        ...
        if e.response.status_code == 404:
            ...
            broken_links.add((current_page_url or 'start', url))
        else:
...
def main():
    ...
    for source_tag, base_url in DOC_SOURCES.items():
        ...
        while urls_to_visit:
            ...
            new_links, found_broken_links = get_all_links(url, base_url, session, current_page_url=source_url)
            all_broken_links.update(found_broken_links)
    ...
    # Insert all collected broken links into the database
    if all_broken_links:
        ...
        logging.info("No broken links found during this scrape.")
